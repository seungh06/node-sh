...
const chmod = (main, ...args) => {
    ...
    if (options.recursive) {
        const child = stdin.map(main => {
            ...
            return node_fs_1.default.statSync(main).isDirectory() ? get_struct(main) : [main];
        }).flat();
        stdin.push(...child);
    }
    stdin.forEach(function processor(input) {
        ...
        const stats = node_fs_1.default.statSync(input), is_dirs = stats.isDirectory();
        let permission = stats.mode, type = permission & node_fs_1.default.constants.S_IFMT;
        ...
        if (typeof mode !== 'number') {
            for (const symbol of mode.split(',')) {
                ...
                if (permission !== new_permission) {
                    ...
                    node_fs_1.default.chmodSync(input, new_permission);
...
function load_reference(target) {
    try {
        return node_fs_1.default.statSync(target).mode;
    }
    catch {
...
function get_struct(main) {
    const res = [];
    node_fs_1.default.readdirSync(main, { withFileTypes: true }).forEach(dirent => {
